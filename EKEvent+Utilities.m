...
//Event Start Date
    ...
    //recurrenceRule-frequency(oldway)
    /*
    if (self.recurrenceRule.frequency == 0) {
        [iCalString appendString:@"\r\nRRULE:FREQ=DAILY"];
    }
    if (self.recurrenceRule.frequency == 1) {
        [iCalString appendString:@"\r\nRRULE:FREQ=WEEKLY"];
    }
    if (self.recurrenceRule.frequency == 2) {
        [iCalString appendString:@"\r\nRRULE:FREQ=MONTHLY"];
    }
    if (self.recurrenceRule.frequency == 3) {
        [iCalString appendString:@"\r\nRRULE:FREQ=YEARLY"];
    }

    if (self.recurrenceRule.interval) {
        [iCalString appendFormat:@";INTERVAL=%d",self.recurrenceRule.interval];
    }
   */
